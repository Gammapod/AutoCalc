...
export const runUiModuleAlgebraicRendererV2Tests = (): void => {
  ...
  try {
    ...
    assert.match(panel.textContent ?? "", /\u{1D453}\u2080\(\u{1D465}\)\s*=\s*_/u, "seedless state shows 𝑓₀(𝑥)=_");
    assert.match(
      panel.textContent ?? "",
      /\u{1D453}\u2099\u208A\u2081\(\u{1D465}\)\s*=\s*\u{1D453}\u2099\(\u{1D465}\)/u,
      "empty builder shows identity recurrence",
    );
    ...
    assert.match(committedText, /\u{1D453}\u2080\(\u{1D465}\)\s*=\s*5/u, "pre-roll seed line uses current seed value");
    assert.match(
      committedText,
      /\(\u{1D453}\u2099\(\u{1D465}\)\s*\+\s*1\)\s*(?:×|Ã—|\*)\s*3/u,
    ...
    const stateWithRelevantSimplifiedRoll: GameState = {
      ...
      calculator: {
        ...
        rollEntries: [
          {
            y: r(0n),
            symbolic: { exprText: "((f_n(x)+pi)-pi)", truncated: false, renderText: "f_n(x)" },
          },
        ],
      },
    };
