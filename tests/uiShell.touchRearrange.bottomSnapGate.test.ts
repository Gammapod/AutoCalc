...
export const runUiShellTouchRearrangeBottomSnapGateTests = (): void => {
  ...
  assert.equal(
    canStartTouchRearrange(state, "touch", false, false, "middle"),
    true,
    "touch rearrange is enabled in middle snap",
  );
  assert.equal(
    canStartTouchRearrange(state, "touch", false, false, "bottom"),
    true,
    "touch rearrange is enabled in bottom snap",
  );
  assert.equal(
    canStartTouchRearrange(state, "mouse", false, false, "bottom"),
    false,
    "touch rearrange only starts for touch pointer input",
  );
  assert.equal(
    canStartTouchRearrange(state, "touch", true, false, "bottom"),
    false,
