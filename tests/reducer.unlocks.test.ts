...
const findKeypadIndex = (state: GameState, keyLike: KeyInput): number =>
  state.ui.keyLayout.findIndex((cell) => cell.kind === "key" && cell.key === k(keyLike));

export const runReducerUnlockTests = (): void => {
  const base = initialState();
  assert.equal(base.unlocks.execution[execution("=")], true, "= starts unlocked");
  assert.equal(base.unlocks.slotOperators[op("-")], false, "- starts locked");
  assert.equal(base.unlocks.utilities[utility("UNDO")], false, "UNDO starts locked");
  ...
  assert.equal(totalGatePre.unlocks.valueExpression[valueExpr("4")], false, "digit 4 stays locked below total 40");
  ...
  assert.equal(totalGatePost.unlocks.valueExpression[valueExpr("4")], true, "digit 4 unlocks at total >= 40");
  ...
  assert.equal(
    allocatorPointGateState.unlocks.utilities[utility("\u2190")],
    true,
  ...
  assert.equal(undoFromReturn.unlocks.utilities[utility("UNDO")], true, "UNDO unlocks on first allocator RETURN press");
  ...
  assert.equal(zeroRollUnlocked.unlocks.valueExpression[valueExpr("0")], true, "0 unlocks when roll contains 0");
  ...
  assert.equal(multiplyRunUnlocked.unlocks.slotOperators[op("*")], true, "* unlocks on constant-step run with abs(step) > 1");
  ...
  assert.equal(hashSequenceUnlocked.unlocks.slotOperators[op("#")], true, "# unlocks on exact sequence 47..5 step -7");
  ...
  assert.equal(
    moduloByEuclidEquivalenceUnlocked.unlocks.slotOperators[op("\u27E1")],
    true,
  ...
  assert.equal(state.unlocks.slotOperators[op("+")], true, "plus unlocks on incrementing run of 7");
  ...
  assert.equal(state.unlocks.valueExpression[valueExpr("1")], true, "digit 1 unlocks when first two-digit total is produced");

  let overflowState = initialState();
  for (let i = 0; i < 9; i += 1) {
    overflowState = press(overflowState, "=");
  }
  const eqBeforeOverflow = overflowState.unlocks.execution[execution("=")];
  overflowState = press(overflowState, "=");
  assert.equal(eqBeforeOverflow, true, "equals is unlocked in the =-only runtime");
  assert.equal(overflowState.unlocks.execution[execution("=")], true, "equals remains unlocked after overflow");
  ...
  assert.equal(afterThreeColumns.unlocks.execution[execution("=")], true, "equals unlocks when keypad has at least 3 key slots");
  ...
  assert.equal(afterEquals.unlocks.slotOperators[op("-")], false, "- remains locked without decrementing run");

  state = reducer(state, { type: "ALLOCATOR_ALLOCATE_PRESSED" });
  assert.equal(state.unlocks.utilities[utility("C")], true, "C unlocks on first allocator Allocate press");
