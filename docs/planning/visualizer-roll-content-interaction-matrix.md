...
* `status` indicates planning maturity: `stub` -> `draft` -> `implemented`.
...
| -------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------- | ---------------- | ---------------- | ----------------------------------------------- | ------ |
| `total`        | `total.threshold_marker`              | Threshold marker / numeric HUD cue as total gets close, for example `[2/7] -> [7/7]`.                                                                | `total_at_least`, `total_at_most`                                    | none                                    | partial          | none             | only when within a certain threshold of success | stub   |
| `total`        | `total.domain_chip`                   | Domain-family chip / token; highlight and shake the domain chip after the condition has been met.                                                    | `roll_contains_domain_type`                                          | none                                    | partial          | none             | post-success only                               | stub   |
| `graph`        | `graph.target_y_line`                 | Faint target point (`y = target`); opacity grows when current total is close.                                                                        | `roll_contains_value`, `total_equals`                                | none                                    | partial          | `forecast`       | only when within a certain threshold of success | stub   |
| `graph`        | `graph.trend_band`                    | Smooth graph of function connecting plotted points; opacity grows as roll reaches required length.                                                   | `roll_ends_with_growth_order_run`, `roll_ends_with_incrementing_run` | none                                    | partial          | `forecast`       | only when within a certain threshold of success | stub   |
| `feed`         | `feed.cycle_diameter`                 | Indicators pointing at the max and min Y elements of the cycle.                                                                                      | `roll_cycle_diameter_at_least`                                       | none                                    | binary           | `cycle`          | shows current min and max, but not target       | stub   |
| `feed`         | `feed.cycle_length`                   | Indicators pointing at the X-index values where the cycle starts.                                                                                    | `roll_cycle_period_at_least`                                         | none                                    | binary           | `cycle`          | highlights current length, but not target       | stub   |
| `number_line`  | `number_line.goal_plots`              | Star-shaped points plotted at target values; opacity increases as current total gets close.                                                          | `total_equals`                                                       | none                                    | partial          | none             | only when within a certain threshold of success | stub   |
