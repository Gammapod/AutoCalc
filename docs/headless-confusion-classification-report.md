...
* `docs/functional-spec.md:72` says installed locked keys are press-usable while locked, and immobile.
...
### 4. `unlock_digit_1_portable_on_total_equals_2` is misleading
...
* `src/content/unlocks.catalog.ts:22` names the unlock `unlock_digit_1_portable_on_total_equals_2`.
...
2. Rename `unlock_digit_1_portable_on_total_equals_2` or change its predicate/description so ID and behavior agree.
