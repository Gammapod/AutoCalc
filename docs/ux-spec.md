...
- `UX-VIS-06` (MUST): Cycle-start highlight gating is canonical and parity-consistent across cycle-highlight visualizers (`total`, `feed`, `ratios`): highlight only when Cycle is enabled (`settings.cycle = "on"`), cycle metadata exists with `stopReason="cycle"`, the latest committed row index is at/after cycle detection index `j`, and the latest committed value equals cycle-start value `f_i`.
